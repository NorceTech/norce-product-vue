...
- **`storefront`** — storefront only, no basket/checkout code (*Developer Fundamentals: Storefront*). This branch.
- **`main`** — full demo incl. basket, NCO and non-PSP payment (*Developer Fundamentals: Checkout*). Default branch.

Changes to shared code (BFF product endpoints, `ProductCard`, `useHelpers`, types, locales) need porting between the branches by hand — they have diverged before.
